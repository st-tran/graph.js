...
const drawEdge = (weight, sourcePos, targetPos, canvasCtx, options) => {
    ...
    if (options.directed) {
        ...
        canvasCtx.lineTo(tPosCopy[0] - 10 * Math.cos(angle - Math.PI / 6), tPosCopy[1] - 10 * Math.sin(angle - Math.PI / 6));
        canvasCtx.lineTo(...tPosCopy);
        canvasCtx.lineTo(tPosCopy[0] - 10 * Math.cos(angle + Math.PI / 6), tPosCopy[1] - 10 * Math.sin(angle + Math.PI / 6));
    ...
    canvasCtx.fillText(weight, (sourcePos[0] + targetPos[0])/2, (sourcePos[1] + targetPos[1])/2);
};
